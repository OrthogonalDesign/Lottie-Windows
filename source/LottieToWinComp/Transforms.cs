...
namespace Microsoft.Toolkit.Uwp.UI.Lottie.LottieToWinComp
{
    ...
    static class Transforms
    {
            ...
            [NotNullWhen(true)] out CompositionContainerShape? contentsNode)
        {
            ...
            if (inProgress > 1 || outProgress <= 0 || inProgress >= outProgress || layerOpacity.IsAlways(Opacity.Transparent))
            {
        ...
        {
            // Implement opacity for the layer.
            if (opacity.IsAnimated || opacity.InitialValue < Opacity.Opaque)
            {
